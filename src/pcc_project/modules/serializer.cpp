...
void deserialize_bMat_fromStream(cv::Mat &mat, std::istringstream &iss, int rows, int cols)
{
    ...
    */
    ...
    int total_bytes_needed = (total_elements + 7) / 8;  // 理论需要的字节数
    int cnt = 0;
    char code = 0;

    // 1. 读取第一个字节，检查是否成功
    if (!iss.read(&code, sizeof(code))) {
        ROS_ERROR("deserialize_bMat_fromStream: read first byte failed");
        mat.release();
        return;
    }

    // 2. 遍历解析，同时记录已读取的字节数
    int bytes_read = 1;  // 已读取1字节
    for (int row = 0; row < rows; row++) {
        for (int col = 0; col < cols; col++) {
            if (cnt == 8) {
                // 读取下一个字节，检查是否成功+是否超预期字节数
                if (!iss.read(&code, sizeof(code)) || bytes_read >= total_bytes_needed) {
                    ROS_ERROR("deserialize_bMat_fromStream: insufficient data (needed=%d, read=%d)",
                             total_bytes_needed, bytes_read);
                    mat.release();
                    return;
                }
                cnt = 0;
            ...
            // 解析当前位
            mat.at<int>(row, col) = ((code >> cnt) & 1) ? 1 : 0;
            cnt++;
        }
    }
    // 3. 强制将流指针移动到b_mat结束位置（跳过冗余字节）
    int remaining_bytes = total_bytes_needed - bytes_read;
    if (remaining_bytes > 0) {
        iss.seekg(remaining_bytes, std::ios::cur);  // Offset from current position
        ROS_INFO("[deserialize_bMat_fromStream] Skip redundant bytes: %d bytes (theoretical needed=%d, actually read=%d)",
                remaining_bytes, total_bytes_needed, bytes_read);
    } else if (remaining_bytes < 0) {
        ROS_WARN("[deserialize_bMat_fromStream] Read bytes exceed theoretical value (overread=%d bytes), possible data error", -remaining_bytes);
    }

    ROS_INFO("[deserialize_bMat_fromStream] b_mat parsed successfully: rows=%d, cols=%d", rows, cols);
...
void serializeMatrixToStream(const pcc::Matrix<double> &mat, std::ostringstream &oss)
{
...
void deserializeMatrixFromStream(std::istringstream &iss, pcc::Matrix<double> &mat)
{
    size_t rows;
...
std::vector<char> serializeData(const cv::Mat &b_mat, const int *idx_sizes,
                                ...
                                const std::vector<int> &tile_fit_lengths, const pcc::Matrix<double> &dct_mat)
{
...
bool deserializeData(const std::vector<char> &data,
                     ...
                     std::vector<int> &tile_fit_lengths, pcc::Matrix<double> &dct_mat)
{
...
// 量化并序列化为int类型
void serialize_dct_toStream(const pcc::Matrix<double> &mat, std::ostringstream &oss, const float quantization_step, const cv::Mat &occ_mat, const cv::Mat &b_mat)
{
    size_t rows = mat.size();
...
// 反量化并反序列化为double类型
void deserialize_dct_fromStream(std::istringstream &iss, pcc::Matrix<double> &mat, const float quantization_step, const cv::Mat &occ_mat, const cv::Mat &b_mat)
{
    size_t rows, cols;
    iss.read(reinterpret_cast<char *>(&rows), sizeof(size_t));
    iss.read(reinterpret_cast<char *>(&cols), sizeof(size_t));
                                  ...
                                  const std::vector<int> &tile_fit_lengths, const pcc::Matrix<double> &dct_mat)
{
    ...
    // 计算理论字节数，对比实际字节数
    int bmat_theory_bytes = (b_mat.rows * b_mat.cols + 7) / 8;  // 理论值
    int bmat_actual_bytes = oss.str().size() - now_size;        // 实际值
    ROS_INFO("[q_serializeData] b_mat: rows=%d, cols=%d, theory_bytes=%d, actual_bytes=%d",
            b_mat.rows, b_mat.cols, bmat_theory_bytes, bmat_actual_bytes);
