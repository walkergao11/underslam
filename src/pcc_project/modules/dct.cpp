...
std::pair<pcc::Matrix<double>, pcc::Matrix<bool>> sadct(const pcc::Matrix<double> &data, const pcc::Matrix<bool> &mask)
{
    int rows = data.size();
    int cols = data[0].size();
    pcc::Matrix<double> temp_img(rows, std::vector<double>(cols, 0.0));
    pcc::Matrix<bool> mask_(rows, std::vector<bool>(cols, false));
    ...
    pcc::Matrix<double> temp_img_new(rows, std::vector<double>(cols, 0.0)); // vector -> std::vector
    pcc::Matrix<bool> mask_final(rows, std::vector<bool>(cols, false));     // vector -> std::vector
...
pcc::Matrix<double> saidct(const pcc::Matrix<double> &data, const pcc::Matrix<bool> &mask)
{
    int rows = mask.size();
    int cols = mask[0].size();
    pcc::Matrix<double> coeff(rows, std::vector<double>(cols, 0.0));

    pcc::Matrix<bool> _mask(rows, std::vector<bool>(cols, false));
    pcc::Matrix<bool> _mask1(rows, std::vector<bool>(cols, false));
    pcc::Matrix<std::pair<int, int>> index_mask1(rows, std::vector<std::pair<int, int>>(cols, {-1, -1}));
    pcc::Matrix<std::pair<int, int>> index_mask(rows, std::vector<std::pair<int, int>>(cols, {-1, -1}));
    pcc::Matrix<std::pair<int, int>> index_mask2(rows, std::vector<std::pair<int, int>>(cols, {-1, -1}));
    ...
    // Step 3: Perform IDCT on each row
    pcc::Matrix<double> temp_coeff(rows, std::vector<double>(cols, 0.0));
    for (int i = 0; i < rows; ++i)
    {
    ...
    // Step 4: Map IDCT rows back to column positions
   temp_coeff.assign(rows, std::vector<double>(cols, 0.0));
    for (int i = 0; i < rows; ++i)
    {
        for (int j = 0; j < cols; ++j)
        {
    ...
    // Step 6: Map IDCT columns back to original positions
    pcc::Matrix<double> real_result(rows, std::vector<double>(cols, 0.0)); // vector -> std::vector
    for (int i = 0; i < rows; ++i)
    {
        for (int j = 0; j < cols; ++j)
        {
...
std::pair<pcc::Matrix<double>, pcc::Matrix<bool>> delta_sadct(const pcc::Matrix<double> &data, const pcc::Matrix<bool> &mask)
{
    // make a copy of data
    pcc::Matrix<double> temp_img = data;
    // mean
    double sum = 0;
    int cnt = 0;
    for (int i = 0; i < data.size(); ++i)
...
pcc::Matrix<double> delta_saidct(const pcc::Matrix<double> &data, const pcc::Matrix<bool> &mask)
{
    int rows = mask.size();
    int cols = mask[0].size();
    double scale = 10000;
    ...
    // Step 6: Map IDCT columns back to original positions
    pcc::Matrix<double> real_result(rows, std::vector<double>(cols, 0.0));
    for (int i = 0; i < rows; ++i)
    {
        for (int j = 0; j < cols; ++j)
        {
