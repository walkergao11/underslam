...
namespace decoder
{
  ...
  // ksample version
  void calc_fit_nums(cv::Mat &img, const cv::Vec4f &c, int occ_code, int c_idx,
                     int r_idx, int len_itr, int tile_size, int ksample, pcc::Matrix<float> &extra_pc)
  {
    int itr = 0;
  ...
  double single_channel_decode(cv::Mat &img, cv::Mat &b_mat, const int *idx_sizes,
                               ...
                               int tile_size, pcc::Matrix<double> &dct_mat, int ksample, pcc::Matrix<float> &extra_pc)
  {
    ...
    pcc::Matrix<bool> unfit_mask = pcc::Matrix<bool>(idx_sizes[0] * tile_size, std::vector<bool>(idx_sizes[1] * tile_size, false));
...
#ifdef USE_SADCT
    auto idct_res = pcc::saidct(dct_mat, unfit_mask);
#else
    auto idct_res = pcc::delta_saidct(dct_mat, unfit_mask);
#endif
    for (int r_idx = 0; r_idx < idx_sizes[0]; r_idx++)
    {
      for (int c_idx = 0; c_idx < idx_sizes[1]; c_idx++)
    ...
    std::cout << "unfit_nums size: " << unfit_nums.size() << std::endl;
    ...
    std::cout << "occ mat size: " << occ_mat.rows << " " << occ_mat.cols << std::endl;
    for (int r_idx = 0; r_idx < idx_sizes[0]; r_idx++)
    {
      int len_itr = 0;
      int len = 0;
  ...
  double single_channel_decode(cv::Mat &img, cv::Mat &b_mat, const int *idx_sizes,
                               std::vector<cv::Vec4f> &coefficients, cv::Mat &occ_mat,
                               std::vector<int> &tile_fit_lengths,
                               int tile_size, pcc::Matrix<double> &dct_mat)
  {
